...
def process_video(ws):
    ...
    while True:
        ...
        try:
            ...
            print(f"DEBUG: Frame received - shape: {frame.shape}, type: {frame.dtype}, min: {frame.min()}, max: {frame.max()}, mean: {frame.mean():.2f}")
            
            # Debug: Save first few images for analysis
            if saved_image_count < MAX_IMAGES_TO_SAVE:
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
                filename = f"{DEBUG_DIR}/frame_{saved_image_count:03d}_{timestamp}.jpg"
            ...
            if results.multi_hand_landmarks:
                ...
                for idx, hand_landmarks in enumerate(results.multi_hand_landmarks):
                    ...
                    for lm in hand_landmarks.landmark:
                        ...
                        print(f"DEBUG: Adding keypoint {len(keypoints)}: ({x_px}, {y_px})")
            ...
            print(f"DEBUG: topologyRaw: {topology}, is JSONArray: {isinstance(topology, list)}, length: {len(topology)}")
