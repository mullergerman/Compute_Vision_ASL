...
class TelegrafClient:
    ...
    def __init__(
        ...
        self.payload_format = (
            ...
            .lower()
                ...
                # Support both HTTP (direct telegraf) and HTTPS (nginx proxy)
        if os.getenv("TELEGRAF_USE_HTTPS", "false").lower() == "true":
            self.url = f"https://{self.host}:8443/ingest"
        else:
    ...
    def _metric_to_dict(self, metric: Metric) -> Dict[str, object]:
        ...
        # Flatten the format for Telegraf JSON ingestion
        payload = {"measurement": name}
        
        # Add timestamp as 'ts' if provided
        if ts is not None:
            payload["ts"] = ts
            
        # Add all tags directly to the payload
        payload.update(tags)
        
        # Add all fields directly to the payload  
